...
docker build -q -t "${OR_HAYABUSA_LOCAL_TAG}" /tmp/openrelik-worker-hayabusa \
    && success "hayabusa image built: ${OR_HAYABUSA_LOCAL_TAG}" \
    || error "Failed to build hayabusa worker - check /tmp/openrelik-worker-hayabusa"
rm -rf /tmp/openrelik-worker-hayabusa
...
# OpenRelik unit - After=timesketch.service enforces startup order.
# OpenRelik's compose creates the openrelik_default network; only after
# that network exists can timesketch-web join it. Without this ordering
# the network join races and fails silently on every reboot.
...
echo "  |    openrelik-worker-hayabusa    (EVTX triage, local build)  |"
...
echo "  |  Order: Timesketch first, then OpenRelik             |"
echo "  |                                                      |"
echo "  |  Manual control:                                     |"
...
echo "  +------------------------------------------------------+"
echo ""
echo -e "${CYAN}  STARTUP SCRIPTS (manual fallback)${NC}"
